import React from 'react'
import './css/ContactUs.css';


function ContactUs() {
    return (
        <div class="homeItems">
            <div class="box-home m-3">
                <div class="container-mt-3" id="team-box">
                    <div class="row">
                        <div class="card">
                            <img class="card-img-top" src="https://cdn.discordapp.com/attachments/841329211448360997/841469679876440074/gif.gif"
                                alt="Card image cap"></img>
                            <div class="card-body">
                                <h3 class="card-title">Beatriz Gonçalves</h3>
                                <p class="card-text prop"> Engineering student at ISEL <br></br>(Instituto Politécnico Engenharia de Lisboa)</p>
                                <br></br>
                                <a href="https://github.com/beatrizctgoncalves"> <img id="github-logo"
                                    src="https://img2.gratispng.com/20180709/lgb/kisspng-computer-icons-download-github-clip-art-github-icon-5b44289596fd85.7749119715311934936185.jpg"></img>
                                </a>
                            </div>
                        </div>
                        <div class="card">
                            <img class="card-img-top" src="https://cdn.discordapp.com/attachments/841329211448360997/841469679876440074/gif.gif"
                            alt="Card image cap"></img>
                            <div class="card-body">
                                <h3 class="card-title">Maksym</h3>
                                <p class="card-text prop">Engineering student at ISEL <br></br>(Instituto Politécnico Engenharia de Lisboa)</p>
                                <br></br>
                                <a href="https://github.com/A44866"> <img id="github-logo"
                                    src="https://img2.gratispng.com/20180709/lgb/kisspng-computer-icons-download-github-clip-art-github-icon-5b44289596fd85.7749119715311934936185.jpg"></img>
                                </a>
                            </div>
                        </div>
                        <div class="card">
                            <img class="card-img-top" src="https://cdn.discordapp.com/attachments/841329211448360997/841469679876440074/gif.gif"
                                alt="Card image cap"></img>
                            <div class="card-body">
                                <h3 class="card-title">Miguel Pinto</h3>
                                <p class="card-text prop">Engineering student at ISEL <br></br>(Instituto Politécnico Engenharia de Lisboa)</p>
                                <br></br>
                                <a href="https://github.com/pinto6"><img id="github-logo"
                                    src="https://img2.gratispng.com/20180709/lgb/kisspng-computer-icons-download-github-clip-art-github-icon-5b44289596fd85.7749119715311934936185.jpg"></img>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default ContactUs